...
void ECSCore::_ecs_update(SystemType system_type) {
    ...
    for (auto& callback : valid_callbacks_view) {
        ...
        if (comp_types.empty()) {
            callback->_call(*this, std::nullopt);
            continue;
        }

        // Get the smallest map that has a component listed
        std::type_index min_type = *std::min_element(
            comp_types.begin(),
            comp_types.end(),
        ...
        std::unordered_map<Entity, std::any>& smallest_map = ecs.m_component_map[min_type];

        // spdlog::info("reached {}", smallest_map.size());
